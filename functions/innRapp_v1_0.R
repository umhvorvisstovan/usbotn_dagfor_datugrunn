...
innRapp_v1.0 <- function(rapport, alioki, filetoimport) {
  ...
  synikanningarfolk <- read_excel(filetoimport,
                                  ...
                                  col_types = c("text", "text", "text"))
  #broyting frá v1.0
  nmsbotndjor <- names(read_excel(filetoimport, sheet = "BotnDjor"))
  botndjor <- read_excel(filetoimport,
                         sheet = "BotnDjor",
                         col_types = c(rep("text", length(nmsbotndjor))))
  ...
  #broyting galdandi frá v1.0
  #Hetta er tí at vidd_upptald_fraktion bleiv broytt til fraktion_upptald í excel skjalinum
  djoralivsyni <- djoralivsyni %>% rename(vidd_upptald_fraktion = fraktion_upptald)
  #---
  ...
  #kanna fyri duplicate í djóraslag, henda kanningin má gerðast áðrenn urslitdjoraliv talvan verður broytt!!
  ...
  if(nrow(urslitkemi_duplicat)>0) {
    warning(paste("tað eru duplicat kemiúrslitunum, eru hetta verðulig duplicat ella feilir?"))
  }
  ...
  if(nrow(djoralivsyni)>0) {
    ...
    if(nrow(urslitdjoraliv_duplicat)>0) {
      stop(paste("tað eru duplicat í djorasløg! aphia_id = ", list(unique(urslitdjoraliv_duplicat$aphia_id))))
    }
  }
  ...
  dbevjutjugd <- tbl(con, "evjutjugd") %>% select(evjutjugd_id) %>% collect()
  fk_evjutjugd <- anti_join(urslitsens, dbevjutjugd, by = c("evjutjugd" = "evjutjugd_id"))%>% select(evjutjugd) %>% filter(!is.na(evjutjugd))
  if(nrow(fk_evjutjugd)>0) {
    dbDisconnect(con)
    stop(paste("Evjutjugd ikki registrerað í datagrunni, evjutjugd_id = ", list(unique(fk_evjutjugd$evjutjugd))))
  }
  
  dbgrabbanogd <- tbl(con, "grabbanogd") %>% select(grabbanogd_id) %>% collect()
  fk_grabbanogd <- anti_join(urslitsens, dbgrabbanogd, by = c("grabbanogd" = "grabbanogd_id"))%>% select(grabbanogd) %>% filter(!is.na(grabbanogd))
  if(nrow(fk_evjutjugd)>0) {
    dbDisconnect(con)
    stop(paste("Grabbanogd ikki registrerað í datagrunni, grabbanogd_id = ", list(unique(fk_grabbanogd$grabbanogd))))
  }
  
  #----
  #broyting galdandi frá v1.0
  dbkanningarstarvsstova <- tbl(con, "kanningarstarvsstova") %>% select(starvsstova) %>% collect()
  fk_starvsstova <- anti_join(urslitkemi, dbkanningarstarvsstova, by = "starvsstova") %>% select(starvsstova)
  if(nrow(fk_starvsstova)>0) {
    dbDisconnect(con)
    stop(paste("Kanningar starvsstova ikki registrerað í datagrunni, starvsstova = ", list(unique(fk_starvsstova$starvsstova))))
  }
  
  dbmethodmatiovissa <- tbl(con, "methodmatiovissa") %>% select(met_matiovissa) %>% collect()
  fk_met_matiovissa <- anti_join(urslitkemi, dbmethodmatiovissa, by = "met_matiovissa") %>% select(met_matiovissa) %>% filter(!is.na(met_matiovissa))
  if(nrow(fk_met_matiovissa)>0) {
    dbDisconnect(con)
    stop(paste("Óvissu rokni háttur er ikki registrerað í datagrunni, met_matiovissa = ", list(unique(fk_met_matiovissa$met_matiovissa))))
  }
  #----
